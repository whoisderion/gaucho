...
app.post("/setup/fleets", async (req: Request, res: Response) => {
	...
	try {
		...
		async function clearDB() {
			...
			console.log(
				"fleet setup: fleets:",
				clearFleets.count + " vehicles:",
				clearVehicles.count + ", inventory:",
				clearInventory.count + ", equipment:",
				clearEquipment.count,
				"equipmentInventory:",
				clearEI.count
			)
		}
	...
	for await (const currTruck of inventory) {
		...
		async function createInventoryRecord() {
			...
			for (const [key, value] of Object.entries(currTruck.inventory)) {
				const currItemName = equipmentTypes.find(
					(obj: { name: string; id: any }) => obj.id === value.equipmentTypeID
				).name
	...
	const generateQR = async () => {
		try {
			const imgURL = await QRCode.toDataURL(
				`${process.env.CLIENT_URL}trucks/${truckID}`
			)
			console.log(imgURL)
			res.write(imgURL)
		} catch (err) {
...
app.get(
	"/upload/equipment-fields/:companyID",
	async (req: Request, res: Response) => {
		const companyID = req.params.companyID
		try {
	...
	try {
		const maintenanceUpdate = await prisma.maintenance.create({
			data: {
				...
				frontDriverTread:
					String(data.maintenance.frontDriverTread) ?? undefined,
				frontPassengerTread:
					String(data.maintenance.frontPassengerTread) ?? undefined,
				rearDriverTread: String(data.maintenance.rearDriverTread) ?? undefined,
				rearPassengerTread:
					String(data.maintenance.rearPassengerTread) ?? undefined,
		...
		let equipmentItems = []
		for (const [equipmentName, equipmentValues] of Object.entries(
			data.equipment
		)) {
			equipmentItems.push({
				quantity: equipmentValues.quantity,
				equipmentId: equipmentValues.id,
			})
		}
...
app.get(
	"/account/company/photo-areas/:companyID",
	async (req: Request, res: Response) => {
		const companyID = req.params.companyID
...
app.post(
	"/account/company/photo-areas",
	async (req: Request, res: Response) => {
		const photoAreasArr = req.body.tempPhotoAreas
		const companyId = req.body.companyId
		const areasToDelete = req.body.areasToDelete
		try {
			const updatedPhotoAreas = await Promise.all(
				photoAreasArr.map(async (area: PhotoAreas) => {
...
app.post("/account/fleet", async (req: Request, res: Response) => {
	...
	try {
		const updatedFleets = await Promise.all(
			tempFleets.map(async (fleet: Fleet) => {
...
app.get("/dashboard/:companyId", async (req: Request, res: Response) => {
	...
	for (const vehicle of vehicles as DashboardVehicle[]) {
		const currFleetIndex = dashboardData.findIndex(
			(currentFleet: Fleet) => currentFleet.id === vehicle.fleetId
		)
	...
	for (const inventory of inventoryList) {
		...
		if (currInventory) {
			for (
				let fleetIndex = 0;
				fleetIndex < dashboardData.length;
				fleetIndex++
			) {
				...
				if (
					dashboardData[fleetIndex].vehicles &&
					dashboardData[fleetIndex].vehicles.findIndex(
						(vehiclesToAdd: Truck) => vehiclesToAdd.id === currInventory.truckId
					) != -1
				) {
					const vehicleIndex = dashboardData[fleetIndex].vehicles.findIndex(
						(vehiclesToAdd: Truck) => vehiclesToAdd.id === currInventory.truckId
					)
					dashboardData[fleetIndex].vehicles[vehicleIndex].inventory =
						currInventory
					...
					if (
						...
						inventory.inventory[1].equipmentItems.length != 0
					) {
						dashboardData[fleetIndex].vehicles[vehicleIndex].lastInventory =
							inventory.inventory[1]
					}
				}
			}
		...
		if (currMaintenance) {
			for (
				...
				if (
					...
					dashboardData[fleetIndex].vehicles.findIndex(
						(vehiclesToAdd: Truck) =>
							vehiclesToAdd.id === currMaintenance.truckId
					) != -1
				) {
					const vehicleIndex = dashboardData[fleetIndex].vehicles.findIndex(
						(vehiclesToAdd: Truck) =>
					...
					dashboardData[fleetIndex].vehicles[vehicleIndex].maintenance =
						currMaintenance
				}
			}
		}
