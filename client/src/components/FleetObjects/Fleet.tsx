...
function findEquipmentByID(equipmentID: number, equipmentArr: Equipment[]) {
    const selectedEquipment = equipmentArr.find(equipment => equipment.id === equipmentID)
    ...
    if (handleFleetNameChange && deleteFleet && deleteVehicle && createNewVehicle && handleVehicleChange) {
        return (
            <div className="pl-12">
                ...
                <button onClick={() => { deleteFleet(currFleet) }}>Delete {fleet.name}</button>
                <hr />
                ...
                <ul>
                    {fleet.vehicles.map((vehicle) => (
                        <div className="flex" key={vehicle.id}>
                                ...
                                <div className="vehicle-equipment ml-8">
                                    ...
                                    {vehicle.equipment.map((equipment) => {
                                        if (equipment.equipmentTypeID) {
                                            const currEquipment = findEquipmentByID(equipment.equipmentTypeID, equipmentTypes)
                                            if (currEquipment) {
                                                return (
                            ...
                            <div>
                                <button onClick={() => { deleteVehicle(vehicle.id, fleet.id) }}>Delete</button>
                            </div>
                        </div>
                    ))}
                    <button onClick={() => { createNewVehicle(fleet.id) }}>Add Vehicle</button>
                </ul>
            </div>
        )
    }
