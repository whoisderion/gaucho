...
function CreateFleet() {
	const dummyFleet = [
		{
			...
			vehicles: [
				...
				},
				{
					...
					vinNumber: "789012",
					id: 2,
					equipment: [{ equipmentTypeID: 1, quantity: 2 }],
				},
			],
			id: 1,
		},
	...
	const [equipmentTypes, setEquipmentTypes] =
		useState<Equipment[]>(dummyEquipment)
	...
	const selectedFleet = fleets.find((fleet) => fleet.id === currFleet)

	function createNewFleet() {
		setCurrFleet(lastFleet.id + 1)
		setFleets([
			...fleets,
			{ name: `Fleet ${lastFleet.id + 1}`, vehicles: [], id: lastFleet.id + 1 },
		])
	}

	function handleFleetNameChange(
		id: number,
					...
					const newFleet = fleet.vehicles.map((vehicle) => {
						if (vehicle.id === vehicleId) {
							if (equipmentQuantity === Number(0)) {
								const index = vehicle.equipment.findIndex(
									(equipment) =>
										equipment.equipmentTypeID === currEquipmentTypeID
								)
							...
							if (type === "changeEquipmentQuantity" && equipmentQuantity) {
								const newVehicleEquipment = vehicle.equipment.map(
									(equipment) => {
								...
								const newVehicleEquipment = [
									...vehicle.equipment,
									{ equipmentTypeID: currEquipmentTypeID, quantity: Number(0) },
								]
								return { ...vehicle, equipment: newVehicleEquipment }
							} else if (
								type === "removeEquipmentType" &&
								currEquipmentTypeID
							) {
								const newVehicleEquipment = vehicle.equipment.filter(
									(equipment) =>
										equipment.equipmentTypeID != currEquipmentTypeID
								)
								return { ...vehicle, equipment: newVehicleEquipment }
								...
								return {
									...vehicle,
									[type]: (e as unknown as HTMLInputElement | HTMLSelectElement)
										.value,
								}
							}
						} else {
	...
	const lastEqupment = equipmentTypes.reduce((prev, curr) =>
	...
	function addEquipmentType() {
		setEquipmentTypes([
			...equipmentTypes,
			{ name: `New Equipment ${lastEqupment.id + 1}`, id: lastEqupment.id + 1 },
		])
	}

	function deleteEquipmentType(equipmentID: number) {
		setEquipmentTypes(
			equipmentTypes.filter((equipment) => equipment.id != equipmentID)
		)
